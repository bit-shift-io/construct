...
where C: ChatProvider + Clone + Send + Sync + 'static
{
    ...
    let (active_task, agent_name, phase) = {
        ...
        (room.active_task.clone(), room.active_agent.clone(), room.task_phase.clone())
    };
    ...
    if phase == TaskPhase::Planning || phase == TaskPhase::NewProject {
        ...
        let _ = chat.send_notification("🚀 **Starting Execution Phase**").await;
        ...
        let _ = chat.send_notification("ℹ️ Task is already valid or not in Planning/NewProject phase.").await;
    }
