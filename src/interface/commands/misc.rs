    ...
    C: ChatProvider + Clone + Send + Sync + 'static,
{
    if args.trim().is_empty() {
        chat.send_notification(crate::strings::messages::ASK_USAGE)
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
        return Ok(());
    }
    ...
    let feed_id = {
        ...
        guard
            .rooms
            .get(&chat.room_id())
            .and_then(|r| r.feed_event_id.clone())
    };

    let feed = feed_manager.unwrap_or_else(|| {
        Arc::new(Mutex::new(crate::application::feed::FeedManager::new(
            workdir.clone(),
    ...
    loop {
        ...
        tracing::info!(
            "DEBUG: .ask loop check. Result is None? {}. Current Phase: {:?}",
            result.is_none(),
            current_phase
        );
...
pub async fn handle_read(
    ...
    let path = args.trim();
    if path.is_empty() {
        chat.send_notification(crate::strings::messages::READ_USAGE)
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
        return Ok(());
    }
    ...
    match client.read_file(&resolved_path).await {
        Ok(content) => {
            chat.send_message(&crate::strings::messages::file_read_success(
                &resolved_path,
                &content,
            ))
            .await
        ...
        Err(e) => {
            chat.send_notification(&crate::strings::messages::file_read_failed(&e.to_string()))
                .await
                .map_err(|e| anyhow::anyhow!(e))?;
        }
    }
