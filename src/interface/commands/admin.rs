...
pub async fn handle_admin(
    ...
    let is_admin = config
        ...
        .admin
        .iter()
        .any(|a| a.to_lowercase() == sender.to_lowercase());
    if !is_admin {
        let _ = chat
    ...
    if command.trim().starts_with("cd ") {
        ...
        match tool_executor.validate_path(&resolved_path) {
            Ok(safe_path) => {
                ...
                let _ = chat
                    .send_message(&crate::strings::messages::directory_changed_msg(
                        &sanitized_path,
                    ))
                    .await;
            }
            Err(e) => {
                let _ = chat
                    .send_notification(&crate::strings::messages::invalid_directory(&e.to_string()))
                    .await;
            }
        }
        return Ok(());
    ...
    match client.execute_command(command, Path::new(&workdir)).await {
        Ok(output) => {
            let sanitized_wd = crate::application::utils::sanitize_path(
                &workdir,
                config.system.projects_dir.as_deref(),
            );
            let _ = chat
                .send_message(&crate::strings::messages::command_output_format(
                    &sanitized_wd,
                    command,
                    &output,
                ))
        ...
        Err(e) => {
            let _ = chat
                .send_notification(&crate::strings::messages::command_failed(&e.to_string()))
                .await;
        }
    }
