...
pub async fn handle_step(
...

    {
        ...
        if input == ".cancel" {
            ...
            if let Some(feed) = &room_state.feed_manager {
                ...
                let _ = chat
                    .send_message(crate::strings::messages::WIZARD_CANCELLED)
                    .await;
            }
            return Ok(WizardAction::Continue);
        }
        ...
        match room_state.wizard.step.clone() {
            Some(WizardStep::ProjectName) => {
                if !input.is_empty() {
                    room_state
                        .wizard
                        .data
                        .insert("name".to_string(), input.to_string());
                ...
                if let Ok(idx) = input.parse::<usize>() {
                    if idx > 0 && idx <= agents.len() {
                        ...
                        room_state
                            .wizard
                            .data
                            .insert("agent".to_string(), selected_agent.to_string());
                        ...
                        let models = match client.list_models(selected_agent).await {
                            ...
                            Err(e) => {
                                tracing::error!(
                                    "Failed to fetch models for {}: {}. Client fallback failed.",
                                    selected_agent,
                                    e
                                );
                                vec![]
                        ...
                        if let Ok(json_str) = serde_json::to_string(&models) {
                            room_state
                                .wizard
                                .data
                                .insert("model_list_cache".to_string(), json_str);
                        }
                        ...
                        let _ = chat
                            .send_message("Invalid selection. Please enter a number.")
                            .await;
                    }
                } else {
            ...
            Some(WizardStep::ModelSelection) => {
                ...
                let models: Vec<(String, String)> =
                    if let Some(cache) = room_state.wizard.data.get("model_list_cache") {
            ...
            if let Some(feed) = &room_state.feed_manager {
                ...
                match &creation_result {
                    Ok(path) => {
                        ...
                        let description_str = room_state
                            .wizard
                            .data
                            .get("description")
                            .cloned()
                        ...
                        let prompt = crate::strings::prompts::new_project_prompt(
                            &name,
                            &description,
                            &sanitized_path,
                            &current_date,
                        ...
                        let display_prompt =
                            format!("Generating documentation for project '{}'.", name);

                        handover_context =
                            Some((prompt, Some(display_prompt), path.clone(), false));
                    }
                    Err(e) => {
    ...
    if let Some((prompt, display_prompt, workdir, create_new_folder)) = handover_context {
        return Ok(WizardAction::TransitionToTask {
            prompt,
            display_prompt,
            workdir,
            create_new_folder,
...
pub async fn start_agent_wizard(
    ...
    let allowed_str = allowed_agents.join(",");
    {
        ...
        room.wizard
            .data
            .insert("available_agents".to_string(), allowed_str);
    }
