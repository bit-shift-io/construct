...
use rig::{completion::Prompt, providers::{gemini, anthropic}};
...
impl Agent for UnifiedAgent {
    async fn execute(&self, context: &AgentContext) -> Result<String, String> {
        ...
        // Model Resolution Logic (keep existing Gemini/Claude logic + others)
        let resolved_model = context.model.clone()
            .or(if !config.model.is_empty() { Some(config.model.clone()) } else { None });
        ...
        match self.provider.as_str() {
            ...
            "claude" | "anthropic" => {
                ...
                let agent = client.agent(&model_name).build();
                agent.prompt(&context.prompt).await.map_err(|e| e.to_string())
            },
            "deepai" | "deep_ai" => {
                ...
                let resp = client.post("https://api.deepai.org/api/text-generator")
                ...
                let body: DeepAIResponse = resp.json().await.map_err(|e| format!("DeepAI Parse Error: {}", e))?;
                Ok(body.output)
            },
            "copilot" | "github_copilot" => {
                ...
                // Refactor to use crate::agent::run_command
                let escaped_prompt = context.prompt.replace("\"", "\\\"");
                let cmd = format!("{}{} \"{}\"", binary, model_flag, escaped_prompt);
                crate::agent::run_command(&cmd, context.working_dir.as_deref()).await
            },
